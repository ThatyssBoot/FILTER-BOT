...
PICS = (environ.get('PICS', 'https://graph.org/file/1d828fbeb4a03531d146c-14424e36fbd6002f45.jpg')).split()


# Admins & Users
ADMINS = [int(admin) if id_pattern.search(admin) else admin for admin in environ.get('ADMINS', '1786851907').split()] # 6272165202 For Multiple Id Use One Space Between Each.
auth_users = [int(user) if id_pattern.search(user) else user for user in environ.get('AUTH_USERS', '6272165202').split()]  # For Multiple Id Use One Space Between Each.
...
CHANNELS = [int(ch) if id_pattern.search(ch) else ch for ch in environ.get('CHANNELS', '-1002488545843').split()]  # For Multiple Id Use One Space Between Each.
...
auth_channel = environ.get('AUTH_CHANNEL', '-1002690816712') # -1002470875163 give your force subscribe channel id here else leave it blank
...
FILE_STORE_CHANNEL = [int(ch) for ch in (environ.get('FILE_STORE_CHANNEL', '-1002488545843')).split()]  # For Multiple Id Use One Space Between Each.
...
PAYMENT_QR = environ.get('PAYMENT_QR', 'https://envs.sh/32F.jpg') # payment code picture url.
...
# Links
GRP_LNK = environ.get('GRP_LNK', 't.me/CINEMACOMPANYE')
CHNL_LNK = environ.get('CHNL_LNK', 'https://t.me/+GF7wG55feCAzZjU1')
SUPPORT_CHAT = environ.get('SUPPORT_CHAT', 'CINEMACOMPANYE') # Support Chat Link Without https:// or @
OWNER_LNK = environ.get('OWNER_LNK', 'https://youtube.com/@iqbaleditzzz?si=zfiuuuPhmcyPlE_2')
...
if 'DYNO' in environ:
    ...
    ON_HEROKU = False
URL = environ.get("URL", "https://fellow-celestine-cinima-69c3105c.koyeb.app/")
